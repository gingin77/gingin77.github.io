...
export async function drawScatterPlot() {
  ...
  let { allRepoData } = await getDataForD3();

  try {
    d3.json(filePath).then(staticData => {
      let dataToPlot = []
      evalDataSetForD3(staticData, allRepoData)

      function evalDataSetForD3(staticData, allRepoData) {
        if (allRepoData) {
          dataToPlot = allRepoData;
          console.log('dataToPlot = updated data from GitHub API')
        } else {
          dataToPlot = staticData
